# 🎙 HopOutSound Bot
Telegram-бот, разработанный по заказу клиента для поддержки и развития начинающих специалистов в сфере сведения музыки, работы со звуком и продакшена.
...
Для этого нужно остановить nginx процессы командой 
```bash
sudo systemctl stop nginx
```

Если не работает и говорит, что порт все еще занят, посмотрите процесс командой
...
## 💳 Логика работы бота
1) Пользователь в Telegram выбирает один из пунктов меню.

2) При оплате через Tribute срабатывает вебхук /webhook.

3) Webhook:

- Проверяет подпись HMAC

- Записывает платёж в PostgreSQL

- Если сумма ≥ цены темплейта, отправляет запрос в /send_pdf бота

4) Бот отправляет файл или благодарственное сообщение.
